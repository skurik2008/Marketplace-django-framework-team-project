...
class OrderCreation:
    ...
    @staticmethod
    def add_delivery_data_to_order(buyer: Buyer, delivery_type: str, address: dict) -> None:
        """ Метод добавления адреса и типа доставки. """

        with transaction.atomic():
            order = OrderCreation._get_not_paid_order(buyer=buyer)
            OrderCreation.add_delivery_type_to_order(delivery_type=delivery_type, order=order)
    ...
    def add_payment_data_to_order(buyer: Buyer, payment_type: str) -> None:
        ...
        with transaction.atomic():
            order = OrderCreation._get_not_paid_order(buyer=buyer)
            OrderCreation.add_payment_type_to_order(payment_type=payment_type, order=order, buyer=buyer)
    ...
    @staticmethod
    def add_payment_type_to_order(payment_type: str, order: Order, buyer: Buyer) -> None:
        ...
        order.payment = OrderCreation._get_payment(buyer=buyer, type_of_payment=type_of_payment)
    ...
    def _get_payment(buyer: Buyer, type_of_payment: PaymentType) -> Payment:
        ...
        return Payment.objects.get_or_create(buyer=buyer, payment_type=type_of_payment)[0]
